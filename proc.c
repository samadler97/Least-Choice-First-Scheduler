...
//array weight for each proccess depending on priority
static const int prior_to_weight[40] = { 88761,     71755,     56483,     46273,     36291,
...
//This implements the Linux
//Completely Fair Scheduler
void scheduler(void){
	
	int sched_latency = 2048;
	int min_gran = 2;
	float weight = 0;
	
	//find current running process
	//and put it in RUNNABLE
	...
	for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
		if (p->pid > 0) {
			float temp_weight = prior_to_weight[p->nice + 20];
			weight += temp_weight;
		}
	}

	for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
		if(p->state == RUNNABLE && !p->hasRun) {
			float temp_weight = prior_to_weight[p->nice + 20];
			p->time_slice =  (temp_weight / weight) * sched_latency;
			p->time_slice = p->time_slice < min_gran ? min_gran : p->time_slice;
			
			//p->vruntime = p->vruntime + ((1024/temp_weight) * p->time_slice);	
		}
	}

	int temp_runtime = 2048;

	for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
		if(p->pid > 0 && !p->hasRun) {
			if (p->time_slice < temp_runtime) {
				temp_runtime = p->time_slice;
				curr_proc = p;
			}
			else if (p->time_slice == temp_runtime) {
				int temp_nice = p->nice;
				if (p->nice < temp_nice) {
					temp_nice = p->nice;
					curr_proc = p;
				}
...
procdump(void)
{
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
    ...
    //CHANGED BY US
    //print the nice value so user can see
